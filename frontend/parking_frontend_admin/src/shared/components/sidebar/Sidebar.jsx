  ...
  export default function Sidebar({ pendingApproval = 0, isOpen = false, onClose }) {
    ...
    const [chatListPos, setChatListPos] = useState({ left: 0, bottom: 0 })
    ...
    const renderLink = (item) => {
      ...
      if (item.id === 'chat') {
        ...
        return (
          <li key={item.id} className="sidebar__item">
            <button ref={chatBtnRef} className="sidebar__link" onClick={handleChatClick}>
              <Icon />
              <span className="sidebar__link-text">{item.label}</span>
              {totalUnread > 0 && <span className="sidebar__badge">{totalUnread}</span>}
            </button>
          </li>
        )
      }
      ...
      <>
        ...
        {showChatList && (
          <ChatRoomList
            ...
            position={chatListPos}
            excludeRef={chatBtnRef}
            onOpenChat={handleOpenChat}
            onLeave={(roomId) => {
        ...
        {openChats.map((chat, index) => (
          <ChatWindow
            ...
            index={index}
            onClose={() => handleCloseChat(chat.roomId)}
            onLeave={() => handleLeaveChat(chat.roomId)}
          />
        ))}
      </>
    )
