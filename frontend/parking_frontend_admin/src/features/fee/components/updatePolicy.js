...
export const updatePolicy=async({title,type,updateMutateAsync})=>{
    ...
    const result = await Swal.fire({
        title: `${title}`,
        width: '500px', // addTicketPolicy와 동일하게 맞춤
        html: `
            <div class="policy-form-container">
                <div class="policy-field">
                    <div class="field-label">회차시간(분)</div>
                    <input type="number" id='graceMinutes' placeholder="예: 30">
                </div>
                
                <div class="policy-field">
                    <div class="field-label">기본요금(원)</div>
                    <input type="number" id='baseFee' placeholder="예: 1000">
                </div>

                <div class="policy-field">
                    <div class="field-label">추가 단위시간(분)</div>
                    <input type="number" id='unitMinutes' placeholder="예: 10">
                </div>

                <div class="policy-field">
                    <div class="field-label">추가 단위요금(원)</div>
                    <input type="number" id='unitFee' placeholder="예: 500">
                </div>

                <div class="policy-field">
                    <div class="field-label">일 최대요금(원)</div>
                    <input type="number" id='daliyMaxFee' placeholder="예: 20000">
                </div>

                <div class="policy-field">
                    <div class="field-label">적용 시작일시</div>
                    <input type="datetime-local" id='effectiveFrom' min="${minDateString.replace(' ', 'T').slice(0, 16)}">
                </div>
            </div>
        `,
        focusCancel: true,
        ...
        preConfirm: () => {
            const data = {
                ...
                effectiveFrom: document.getElementById('effectiveFrom').value ? document.getElementById('effectiveFrom').value + ":00" : ""
            }
            ...
            if (!data.effectiveFrom) {
                Swal.showValidationMessage("적용 시작일시를 선택해주세요.");
                return false;
            }
