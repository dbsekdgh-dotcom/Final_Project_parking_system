import { useState, useCallback, useEffect } from 'react';
import { fetchUsers, fetchUserDetail } from '../api/ManagementApi';
import Pagination from '../../../shared/components/pagination/Pagination';
import { SearchIcon, formatDate } from '../components/ManagementHelpers.jsx';
import UserDetailModal from '../components/UserDetailModal';
import '../../approval/approval-request/pages/ApprovalRequestPage.css';
import '../pages/management.css';

const STATUS_OPTIONS = [
  { value: '',        label: '상태 전체' },
  { value: 'ACTIVE',  label: '활성'     },
  { value: 'DELETED', label: '탈퇴'     },
];
const TYPE_OPTIONS = [
  { value: '',      label: '유형 전체' },
  { value: 'true',  label: '입주민'   },
  { value: 'false', label: '일반'     },
];
const STATUS_LABEL = { ACTIVE: '활성', DELETED: '탈퇴' };

export default function UserVehicleUser() {
  const [rows, setRows]             = useState([]);
  const [loading, setLoading]       = useState(false);
  const [page, setPage]             = useState(0);
  const [totalPages, setTotalPages] = useState(0);
  const [keyword, setKeyword]       = useState('');
  const [status, setStatus]         = useState('');
  const [isResident, setIsResident] = useState('');
  const [detail, setDetail]         = useState(null);
  const [detailLoading, setDetailLoading] = useState(false);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await fetchUsers({ keyword, status, isResident, page, size: 10 });
      setRows(data.content ?? []);
      setTotalPages(data.totalPages ?? 0);
    } finally {
      setLoading(false);
    }
  }, [keyword, status, isResident, page]);

  useEffect(() => { load(); }, [load]);

  const handleRowClick = async (userId) => {
    setDetailLoading(true);
    try {
      const data = await fetchUserDetail(userId);
      setDetail(data);
    } finally {
      setDetailLoading(false);
    }
  };

  const reset = () => { setKeyword(''); setStatus(''); setIsResident(''); setPage(0); };

  return (
    <div className="arp__content">
      <div className="arp__filter">
        <div className="arp__search-wrap">
          <SearchIcon />
          <input
            className="arp__search"
            placeholder="이름, 이메일 검색..."
            value={keyword}
            onChange={(e) => { setKeyword(e.target.value); setPage(0); }}
          />
        </div>
        <div className="arp__divider" />
        <select className="arp__select" value={status}
          onChange={(e) => { setStatus(e.target.value); setPage(0); }}>
          {STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <select className="arp__select" value={isResident}
          onChange={(e) => { setIsResident(e.target.value); setPage(0); }}>
          {TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
        </select>
        <div className="arp__divider" />
        <button className="arp__reset-btn" onClick={reset}>초기화</button>
      </div>

      <div className="arp__table-wrap">
        {loading ? (
          <div className="arp__empty">불러오는 중...</div>
        ) : (
          <table className="arp__table">
            <thead>
              <tr>
                <th>이름</th><th>이메일</th><th>전화번호</th>
                <th>유형</th><th>가입일</th><th>상태</th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 ? (
                <tr><td colSpan={6} className="arp__empty">검색 결과가 없습니다.</td></tr>
              ) : rows.map(row => (
                <tr key={row.userId} className="clickable" onClick={() => handleRowClick(row.userId)}>
                  <td className="arp__td--bold">{row.name}</td>
                  <td className="arp__td--muted">{row.email}</td>
                  <td className="arp__td--mono">{row.phone}</td>
                  <td>{row.type}</td>
                  <td className="arp__td--mono">{formatDate(row.createdAt)}</td>
                  <td>
                    <span className={`arp__status-badge arp__status-badge--${row.status.toLowerCase()}`}>
                      {STATUS_LABEL[row.status] ?? row.status}
                    </span>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        )}
      </div>

      <Pagination page={page} totalPages={totalPages} onPageChange={setPage} />

      {detailLoading && (
        <div className="mgmt-modal-overlay">
          <div className="arp__empty" style={{ color: '#fff' }}>불러오는 중...</div>
        </div>
      )}
      {detail && <UserDetailModal data={detail} onClose={() => setDetail(null)} />}
    </div>
  );
}
