...
export const getStoreMe = async () =>{
    const res = await storeApi.get(`${host}/me`, { headers: authHeader() });
    return res.data;
};

// 구매 가능한 ticket_policy 목록
export const getTicketPolicies = async () => {
    const res = await storeApi.get(`${host}/tickets/policies`, { headers: authHeader() });
    return res.data;
};
// 현재 보유 지갑 목록
export const getWallets = async () => {
    const res = await storeApi.get(`${host}/wallets`,{ headers: authHeader() });
    return res.data;
};
// 구매 준비
