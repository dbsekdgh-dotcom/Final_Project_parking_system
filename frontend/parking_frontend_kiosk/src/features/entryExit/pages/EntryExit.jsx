...
export default function EntryExit() {
  ...
  const { mutate: ocrMutate, isPending: ocrLoading, isError: ocrError } = usePlateOCRMutation();
  const { mutate: entryMutate, isPending: entryLoading } = useEntryMutation();
  ...
  const handleFileChange = (e) => {
    ...
    ocrMutate(file, {
      onSuccess: async (data) => {
        const plate = data?.plate_number ?? data?.plateNumber ?? data?.plate ?? "";
        const s3path = data?.s3path ?? "";
        if (!plate) { alert("번호판 인식 실패"); return; }
      ...
      onError: (err) => { console.error("OCR 실패:", err); alert("OCR 요청 실패"); },
    });
  };

  const handleEntryCamera = (entryCameraId) => {
    if (!plateNumber) { alert("먼저 차량 사진을 업로드하세요."); return; }
    entryMutate(
  ...
  const handleExitCamera = async (exitCameraId) => {
    if (!uploadFile) { alert("먼저 차량 사진을 업로드 하세요."); return; }
    if (!parkingLogId) { alert("차량 정보를 확인할 수 없습니다."); return; }
    setExitLoading(true);
    try {
      const data = await requestExit(parkingLogId, exitCameraId, imagePath);
      if (data.isFree) {
        navigate("/exit-departure", { state: { parkingLogId, message: data.message } });
      } else {
            ...
            {plateChars.length > 0 && (
              <div className="plate-underline">
                {plateChars.map((ch, idx) => (
                  <div key={idx} className="plate-slot">{ch}</div>
                ))}
              </div>
            )}
            ...
            {isEntered === false && (
              <>
                ...
                {entryCameras.map((cam) => (
                  ...
                  >
                    {entryLoading ? "처리 중..." : cam.description || cam.location || `${cam.cameraId}번 입구`}
                  </button>
                ))}
              </>
            )}
            ...
            {isEntered === true && (
              <>
                ...
                {exitCameras.map((cam) => (
                  ...
                  >
                    {exitLoading ? "처리 중..." : cam.description || cam.location || `${cam.cameraId}번 출구`}
                  </button>
                ))}
              </>
            )}
