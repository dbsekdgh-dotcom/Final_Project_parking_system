...
from .tools import (
    get_parking_summary, get_my_reservations, get_my_points,
    create_reservation, cancel_reservation, get_my_subscriptions,
...
금지5. 도구 호출 전에 "잠시만 기다려 주세요", "예약을 진행하겠습니다" 같은 예고 문구를 출력하지 마세요. 결과만 바로 전달하세요.
...
=== 방문 예약 취소 ===
- 예약 목록 조회 후 status 값이 정확히 "PENDING" 또는 "RESERVED"인 항목만 표시하세요. PENDING(승인대기)도 취소 가능합니다.
- status가 CANCELLED, NO_SHOW, COMPLETED, ENTERED, REJECTED인 항목은 목록에서 제외하세요.
- PENDING이나 RESERVED인 항목이 하나도 없을 때만 "취소 가능한 예약이 없습니다."라고 안내하세요.
...
- 사용자가 번호로 선택하면 날짜 계산 없이 즉시 cancel_reservation을 호출하세요. 취소 가능 여부를 스스로 판단하지 마세요.
