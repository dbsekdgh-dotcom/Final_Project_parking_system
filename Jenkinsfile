                        ...
                        script {
                            def cmdId = sh(script: """
                                aws ssm send-command \
                                    ...
                                    --parameters '{"commands":["export HOME=/root && git config --global --add safe.directory /home/ssm-user/Final_Project_parking_system && cd /home/ssm-user/Final_Project_parking_system && git checkout docker-compose.yml && git pull https://${GIT_TOKEN}@github.com/dbsekdgh-dotcom/Final_Project_parking_system.git develop && aws s3 cp s3://parking-frontend-admin/server.env .env && aws ecr get-login-password --region ap-northeast-2 | sudo docker login --username AWS --password-stdin ${ECR_REGISTRY} && sudo docker rm -f parking-backend parking-ai ; sudo docker image prune -af && sudo docker pull ${ECR_REGISTRY}/parking-backend:latest && sudo docker pull ${ECR_REGISTRY}/parking-ai:latest && sudo docker compose -f docker-compose.server2.yml up -d"]}' \
                                    --region ${AWS_REGION} \
                        ...
                        script {
                            def cmdId = sh(script: """
                                aws ssm send-command \
                                    ...
                                    --document-name "AWS-RunShellScript" \
                                    --parameters '{"commands":["export HOME=/root && git config --global --add safe.directory /home/ssm-user/Final_Project_parking_system && cd /home/ssm-user/Final_Project_parking_system && git checkout docker-compose.yml && git pull https://${GIT_TOKEN}@github.com/dbsekdgh-dotcom/Final_Project_parking_system.git develop && aws s3 cp s3://parking-frontend-admin/server.env .env && aws ecr get-login-password --region ap-northeast-2 | sudo docker login --username AWS --password-stdin ${ECR_REGISTRY} && sudo docker rm -f parking-backend parking-ai ; sudo docker image prune -af && sudo docker pull ${ECR_REGISTRY}/parking-backend:latest && sudo docker pull ${ECR_REGISTRY}/parking-ai:latest && sudo docker compose up -d"]}' \
                                    --region ${AWS_REGION} \
                                    --query 'Command.CommandId' \
                                    --output text
                            """, returnStdout: true).trim()
