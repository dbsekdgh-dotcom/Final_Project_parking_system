...
import com.example.demo.domain.parking.log.service.ParkingFeeCalculator;
...
public class AdminParkingService {
    ...
    public void modifyParkingDiscount(Long parkingLogId, Long ticketPolicyId, String reason, AdminAuthDto adminAuthDto) throws Exception{
        ...
        //관리자용 가상 상점 조회
        Store adminStore = storeRepository.findAdminStoreByKeyword("관리", com.example.demo.domain.system.store.enums.Status.ACTIVE)
                .stream()
                .findFirst()
        ...
        if(parkingLog.getParkingTypeSnapshot().equals(ParkingTypeSnapshot.RESERVATION)){
            if(reservationRepository.getCountbyCarNumber(parkingLog.getCarNumberSnapshot(), com.example.demo.domain.reservation.enums.Status.ENTERED,true)<=0){
        ...
        //상가 할인 합계 계산 [Before]
        int beforeStoreTotal = calculateDiscountSum(allTickets, com.example.demo.domain.payment.ticket.Status.STORE);
        //관리자 할인 합계 계산 [Before]
        ...
        parkingTicketRepository.deleteAll(
                allTickets.stream()
                        .filter(t->t.getStatus() == com.example.demo.domain.payment.ticket.Status.ADMIN)
                        .collect(Collectors.toList())
        );
        ...
        ParkingTicket adminTicket = ParkingTicket.builder()
                ...
                .ticketPolicy(policy)
                .store(adminStore)
                .status(com.example.demo.domain.payment.ticket.Status.ADMIN)
    ...
    private int calculateDiscountSum(List<ParkingTicket> tickets, com.example.demo.domain.payment.ticket.Status status){
    ...
    private void cancelReadyPayments(ParkingLog parkingLog) {
        ...
        List<com.example.demo.domain.payment.enums.PaymentStatus> targetStatuses =
                List.of(com.example.demo.domain.payment.enums.PaymentStatus.READY,
                        com.example.demo.domain.payment.enums.PaymentStatus.FAILED);
        ...
        targets.forEach(payment -> {
            payment.setPaymentStatus(com.example.demo.domain.payment.enums.PaymentStatus.CANCELLED);
...
//      LocalDateTime now = LocalDateTime.now(ZoneId.of("Asia/Seoul"));
        ...
        //상가 할인 합계 계산
        int storeSum = calculateDiscountSum(tickets, com.example.demo.domain.payment.ticket.Status.STORE);
        //관리자 할인 합계 계산
        int adminSum = calculateDiscountSum(tickets, com.example.demo.domain.payment.ticket.Status.ADMIN);
